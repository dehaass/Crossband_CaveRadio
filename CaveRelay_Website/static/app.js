...
function renderLog(kind, sourceEntries) {
  ...
  body.innerHTML = entries.map((entry, index) => {
    const route = entry.source || entry.from_call || entry.destination || entry.to_call || '—';
    const detail = entry.message || entry.info || entry.raw || '—';
    let crc = '—';
    let crcClass = '';
    ...
    const snAverage = entry.sn_average === null || entry.sn_average === undefined ? '—' : Number(entry.sn_average).toFixed(1);
    const columns = kind === 'aprs'
      ? `<td class="time">${escapeHtml(entry.received_at || '—')}</td><td class="transport">${escapeHtml(entry.transport || '—')}</td><td class="route">${escapeHtml(route)}</td><td class="detail">${escapeHtml(detail)}</td>`
      : `<td class="time">${escapeHtml(entry.received_at || '—')}</td><td class="transport">${escapeHtml(entry.transport || '—')}</td><td class="route">${escapeHtml(route)}</td><td class="detail">${escapeHtml(detail)}</td><td class="sn">${escapeHtml(snAverage)}</td><td class="crc ${crcClass}">${crc}</td>`;
...
function openDetails(entry) {
  ...
  const fields = Object.entries(entry).map(([key, value]) => `<div class="detail-field"><dt>${escapeHtml(key)}</dt><dd>${escapeHtml(typeof value === 'string' ? value : JSON.stringify(value, null, 2))}</dd></div>`).join('');
